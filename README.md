...
- (Updated 10/2021) - For DHCP, each of the raspberry pi's are running a copy of ISC DHCP server. 

![ISC DHCP](images/isc-dhcp.png)

This easily allows the ability to set up static IP reservations and /etc/hosts file on the pi, which make the names of the Devices on the Network show up in the DNS query log instead of just IPS (hint: you really want this). 

gen_dhcpd_conf.py  will create all the needed entries in the config. Their log files are easily consumed by Fluentd, so can then be shipped to Loggly.  
